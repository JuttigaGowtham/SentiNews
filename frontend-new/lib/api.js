...
export const fetchCommoditiesDashboard = (isFutures = false) => request(`/commodities-dashboard${isFutures ? "?futures=true" : ""}`);
export const fetchIndicesDashboard = (isFutures = false) => request(`/indices-dashboard${isFutures ? "?futures=true" : ""}`);
export const fetchCurrenciesDashboard = (isFutures = false) => request(`/currencies-dashboard${isFutures ? "?futures=true" : ""}`);
...
/* ── Stock detail ────────────────────────── */
export const fetchStockDetail = (symbol) => request(`/stock/${symbol}/detail`);
export const fetchStockHistory = (symbol, range = "1Y") => request(`/stock/${symbol}/history?range=${range}`);
