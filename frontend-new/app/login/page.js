"use client";

import { useState } from "react";
import { useRouter } from "next/navigation";
import { supabase } from "@/lib/supabase";
import { useAuth } from "@/context/AuthContext";
import { login as loginApi } from "@/lib/api";
import { Eye, EyeOff, TrendingUp } from "lucide-react";

export default function LoginPage() {
    const router = useRouter();
    const { login } = useAuth();
    const [email, setEmail] = useState("");
    const [password, setPassword] = useState("");
    const [showPassword, setShowPassword] = useState(false);
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState("");

    const handleSubmit = async (e) => {
        e.preventDefault();
        setError("");
        setLoading(true);

        try {
            if (supabase) {
                const { data, error: authError } = await supabase.auth.signInWithPassword({
                    email: email.toLowerCase().trim(),
                    password,
                });
                if (authError) throw authError;

                if (data?.session) {
                    login(data.session.access_token, data.session.user?.email || email);
                    router.push("/");
                } else {
                    throw new Error("No session returned from authentication.");
                }
            } else {
                // Fallback to local authentication
                const res = await loginApi(email.toLowerCase().trim(), password);
                if (res?.access_token) {
                    login(res.access_token, email.toLowerCase().trim());
                    router.push("/");
                } else {
                    throw new Error("Invalid response from authentication server");
                }
            }
        } catch (err) {
            setError(err.message || "Invalid credentials or network error");
        } finally {
            setLoading(false);
        }
    };

    const handleGoogleLogin = async () => {
        setError("");
        if (!supabase) {
            setError("Supabase is not configured.");
            return;
        }
        try {
            const { error: authError } = await supabase.auth.signInWithOAuth({
                provider: "google",
                options: {
                    redirectTo: `${window.location.origin}/`,
                },
            });
            if (authError) throw authError;
        } catch (err) {
            setError(err.message || "Failed to start Google sign-in");
        }
    };

    return (
        <div className="auth-page">
            <div className="auth-card">
                {/* Logo Section */}
                <div style={{ display: "flex", justifyContent: "center", marginBottom: "2rem" }}>
                    <div 
                        className="auth-social-pill" 
                        onClick={() => router.push("/")} 
                        style={{ 
                            cursor: "pointer", 
                            width: "100%", 
                            justifyContent: "center", 
                            padding: "0.9rem 1.5rem",
                            background: "var(--bg-elevated)",
                            borderColor: "var(--border-subtle)",
                            fontWeight: "1000",
                            fontSize: "1.5rem",
                            textTransform: "uppercase",
                            letterSpacing: "0.15em"
                        }}
                    >
                        <span className="brand-logo-badge-text">SENTINEWS</span>
                    </div>
                </div>

                <div className="auth-form-content-new" style={{ margin: "0" }}>
                    {/* Google Login Button at the Top */}
                    <button 
                        type="button" 
                        className="auth-social-pill" 
                        onClick={handleGoogleLogin}
                        style={{ width: "100%", justifyContent: "center", gap: "10px", padding: "0.9rem 1.5rem", marginBottom: "1.5rem" }}
                    >
                        <svg viewBox="0 0 24 24" style={{ width: "18px", height: "18px" }}>
                            <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                            <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                            <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z" />
                            <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z" />
                        </svg>
                        <span>Continue with Google</span>
                    </button>

                    {/* Divider */}
                    <div style={{ display: "flex", alignItems: "center", margin: "1.5rem 0", color: "var(--text-muted)", fontSize: "0.85rem" }}>
                        <div style={{ flex: 1, height: "1px", background: "var(--border-subtle)" }} />
                        <span style={{ padding: "0 10px", fontWeight: "500" }}>or continue with email</span>
                        <div style={{ flex: 1, height: "1px", background: "var(--border-subtle)" }} />
                    </div>

                    <form onSubmit={handleSubmit}>
                        <div className="auth-field-group-new">
                            <label className="auth-field-label-new">Email</label>
                            <input
                                type="email"
                                className="auth-input-pill"
                                value={email}
                                onChange={(e) => setEmail(e.target.value)}
                                placeholder="you@example.com"
                                required
                            />
                        </div>

                        <div className="auth-field-group-new">
                            <label className="auth-field-label-new">Password</label>
                            <div className="auth-input-pill-wrapper">
                                <input
                                    type={showPassword ? "text" : "password"}
                                    className="auth-input-pill password-field"
                                    value={password}
                                    onChange={(e) => setPassword(e.target.value)}
                                    placeholder="••••••••••••"
                                    required
                                />
                                <button
                                    type="button"
                                    className="auth-input-toggle-pill"
                                    onClick={() => setShowPassword(!showPassword)}
                                >
                                    {showPassword ? <EyeOff size={18} /> : <Eye size={18} />}
                                </button>
                            </div>
                        </div>

                        {error && (
                            <div className="auth-error-msg" style={{ marginTop: "1rem", borderRadius: "99px", padding: "0.75rem 1.5rem" }}>
                                <span>{error}</span>
                            </div>
                        )}

                        <button className="auth-btn-pill" type="submit" disabled={loading} style={{ marginTop: "1.5rem" }}>
                            {loading ? "Logging in..." : "Login"}
                        </button>
                    </form>
                </div>

                <div className="auth-footer-new" style={{ marginTop: "2rem", justifyContent: "space-between" }}>
                    <span>
                        New User? <button type="button" className="auth-footer-new-link" onClick={() => router.push("/signup")}>Sign up</button>
                    </span>
                    <button type="button" className="auth-footer-new-link" onClick={() => router.push("/forgot-password")}>Forgot password?</button>
                </div>
            </div>
        </div>
    );
}
