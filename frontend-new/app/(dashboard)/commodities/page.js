...
import { fetchCommoditiesDashboard } from "@/lib/api";
import DashboardGrid from "@/components/dashboard/DashboardGrid";
import { FiClock, FiRefreshCw, FiBarChart2 } from "react-icons/fi";
...
export default function CommoditiesPage() {
  ...
  const loadData = useCallback(async (isSilent = false, futuresMode = isFutures) => {
    ...
    try {
      const res = await fetchCommoditiesDashboard(futuresMode);
      setData(res);
      setError("");
    } catch (err) {
      console.error(err);
  ...
  if (loading) return <LoadingScreen message="Syncing Commodities Terminal..." />;
  ...
  return (
    <div className="commodities-container">
      <div className="dashboard-header">
        <div className="header-title">
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <FiBarChart2 className="accent" size={24} />
            <h1>Commodities Terminal</h1>
          </div>
          <p style={{ color: "var(--text-secondary)" }}>
            Real-time Energy, Metals & Agriculture Intelligence
          </p>
        </div>
      ...
      <DashboardGrid data={data} type="commodities" />
...
const thStyle = { padding: "12px 16px", color: "var(--text-secondary)", fontSize: "0.75rem", textTransform: "uppercase" };
const tdStyle = { padding: "16px", borderBottom: "1px solid var(--border-faint)" };

function formatPct(val) { return val == null ? "—" : (val >= 0 ? "+" : "") + val.toFixed(2) + "%"; }
function getHeatStyle(val) {
  if (val == null) return {};
  if (val > 2) return { color: "#10b981", fontWeight: 700 };
  if (val < -2) return { color: "#ef4444", fontWeight: 700 };
  return {};
}
