    ...
    <div style={{ maxWidth: 1200, margin: "0 auto", padding: "1.5rem 1rem", color: "var(--text-primary)" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: "24px" }}>
        <div className="glass-card" style={{ padding: 16 }}>
            <form onSubmit={addTicker} style={{ marginBottom: 16, position: "relative" }} onClick={(e) => e.stopPropagation()}>
                <input
                    value={input}
                    onChange={(e) => setInput(e.target.value)}
                    placeholder="+ Find Stock (e.g. RELIANCE, TCS)"
                  ...
                  <div style={{
                    ...
                    {searchResults.map((item) => (
                      ...
                      >
                        ...
                        <span style={{ color: "var(--text-muted)", fontSize: "0.75rem", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: "70%" }}>{item.name}</span>
                      </div>
                    ))}
                  </div>
                )}
