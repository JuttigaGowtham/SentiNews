    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (supabase) {
                ...
                const res = await signupApi(email.toLowerCase().trim(), password);
                if (res?.access_token) {
            ...
            setError(err.message || "Something went wrong or email is already taken");
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="auth-page">
            <div className="auth-card">
                ...
                <div className="auth-footer-new" style={{ marginTop: "2rem", justifyContent: "center" }}>
                    <span>
                        Already have an account? <button type="button" className="auth-footer-new-link" onClick={() => router.push("/login")}>Log in</button>
                    </span>
                </div>
            </div>
        </div>
